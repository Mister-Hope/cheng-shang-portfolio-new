  ...
  const renderBlock = (block: ContentBlock) => {
    switch (block.type) {
                  ...
                  {block.data.contact.map((c: any, i: number) => (
                    <li key={i} className="flex items-center gap-4 group">
                      <div className="w-14 h-14 rounded-3xl bg-white shadow-sm border border-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                        <i
                          className={`fa-solid ${c.label.toLowerCase().includes("riken") ? "fa-at" : c.label.toLowerCase().includes("utokyo") || c.label.toLowerCase().includes("东京大学") ? "fa-university" : "fa-envelope"} text-lg`}
                        ></i>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-slate-400 text-[9px] font-black uppercase tracking-widest mb-0.5">
                          {c.label}
                        </span>
                        <a
                          href={`mailto:${c.value}`}
                          className="text-slate-800 hover:text-blue-600 font-bold text-lg transition-colors border-b-2 border-transparent hover:border-blue-100"
                        >
                          {c.value}
                        </a>
                      </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                {block.data.interests.map((interest: string, i: number) => (
                  <span
                    key={i}
                    className="px-6 py-3 bg-white border border-slate-100 rounded-3xl text-slate-700 font-black shadow-sm hover:shadow-xl hover:shadow-blue-500/10 hover:-translate-y-1 transition-all cursor-default text-sm"
                  >
                    {interest}
                  </span>
                ))}
              ...
              <div className="mt-12 p-8 bg-gradient-to-br from-blue-50/50 to-indigo-50/50 rounded-[2.5rem] border border-blue-100/50 relative overflow-hidden">
                ...
                <p className="text-sm text-blue-800 font-black leading-relaxed relative z-10 italic">
                  "Theoretical physics is the pursuit of understanding the
                  fundamental structure of reality through logic and beauty."
                </p>
              </div>
            </div>
          </div>
            ...
            {block.data.map((card: any, idx: number) => (
              ...
              >
                ...
                <div className="relative z-10">
                  <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-6">
                    {i18n.thesis}
                  </h4>
      ...
      case "list":
        ...
        return (
          <ListTag
            className={`space-y-6 ${block.config?.listType === "ol" ? "list-decimal list-inside marker:text-blue-400 marker:font-black" : "list-none"}`}
          >
            {block.data.map((item: any, i: number) => (
              <li
                key={i}
                className={`text-slate-700 leading-relaxed ${block.config?.listType !== "ol" ? "flex items-start gap-4" : "pl-2"}`}
              >
                {block.config?.listType !== "ol" && listStyle !== "none" && (
                  <div
                    className={`mt-1.5 w-6 h-6 flex-shrink-0 flex items-center justify-center transition-all ${
                      listStyle === "circle"
                        ? "rounded-full bg-blue-100 border-2 border-white shadow-sm"
                        : listStyle === "square"
                          ? "rounded-xl bg-blue-100 border-2 border-white shadow-sm"
                          : "bg-transparent"
                    }`}
                  >
                    {listStyle === "check" && (
                ...
                <div className="inline flex-1 font-semibold">
                  {item.link ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:text-blue-600 border-b border-transparent hover:border-blue-200 transition-all"
                    >
