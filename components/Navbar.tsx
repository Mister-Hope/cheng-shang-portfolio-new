...
const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${scrolled ? "py-4 glass border-b border-slate-100 shadow-xl shadow-slate-200/20" : "py-8"}`}
    >
      <div className="container mx-auto px-6 max-w-6xl flex items-center justify-between">
        <a
          href="#"
          className={`text-xl font-black tracking-tighter transition-all duration-300 ${scrolled ? "text-slate-900" : "text-white drop-shadow-md"}`}
        >
          {brand}
        </a>
